...
const writeButton = root?.querySelector('[data-inquiry-write]');
...
const firebaseMessage = (error, step = '처리') => {
  const code = String(error?.code || 'unknown');
  if (code.includes('permission-denied')) return `${step} 권한이 없습니다. Firestore Rules 게시 상태를 확인해 주세요. (${code})`;
  if (code.includes('unauthenticated')) return `로그인 상태가 만료되었습니다. 다시 로그인해 주세요. (${code})`;
  if (code.includes('unavailable')) return `Firebase 연결이 원활하지 않습니다. 잠시 후 다시 시도해 주세요. (${code})`;
  if (code.includes('failed-precondition')) return `Firestore 설정을 확인해 주세요. (${code})`;
  return `${step} 중 오류가 발생했습니다. (${code})`;
};
...
const returnUrl = () => `${window.location.pathname.split('/').pop() || 'inquiry.html'}${window.location.search || ''}`;
const goLogin = () => {
...
function publicPayloadFromPrivate(item) {
  return {
    ownerUid: item.userId,
    userName: item.userName || '리림 회원',
    category: item.category || '이용 문의',
    title: item.title || '문의',
    status: item.status || '답변대기',
    createdAt: item.createdAt || Timestamp.now(),
    updatedAt: item.updatedAt || item.createdAt || Timestamp.now()
  };
}

async function repairMyMissingIndexes(user) {
  if (!db || !user || isAdmin(user)) return;
  try {
    const mine = await getDocs(query(collection(db, 'inquiries'), where('userId', '==', user.uid)));
    for (const inquiryDoc of mine.docs) {
      const indexRef = doc(db, 'inquiryIndex', inquiryDoc.id);
      const indexDoc = await getDoc(indexRef);
      if (!indexDoc.exists()) {
        await setDoc(indexRef, publicPayloadFromPrivate(inquiryDoc.data()));
      }
    }
  } catch (error) {
    console.warn('[RE:LIM Q&A] 누락 목록 복구 보류', error);
  }
}
...
async function openInquiry(indexItem, syncUrl = true) {
  ...
  if (!isAdmin(user) && indexItem.ownerUid !== user.uid) return showPrivacyNotice(false);
  ...
  try {
    ...
    console.error('[RE:LIM Q&A] 상세 조회 실패', error);
    if (String(error?.code || '').includes('permission-denied')) return showPrivacyNotice(false);
    window.alert(firebaseMessage(error, '문의 조회'));
  }
}
...
if (db) {
  onSnapshot(
    ...
    renderBoard,
    (error) => {
      console.error('[RE:LIM Q&A] 목록 조회 실패', error);
      if (empty) {
        empty.hidden = false;
        empty.textContent = firebaseMessage(error, '문의 목록 조회');
      }
    }
  );
}
...
if (auth) {
  onAuthStateChanged(auth, (user) => {
    currentUser = user;
    openedFromUrl = false;
    if (user) repairMyMissingIndexes(user);
  });
}
...
writeForm?.addEventListener('submit', async (event) => {
  ...
  setStatus(writeStatus, '문의 내용을 저장하고 있습니다.');
  ...
  try {
    // 실제 문의 내용부터 저장한다. 이후 목록 저장이 실패해도 문의 기록 자체는 남긴다.
    await setDoc(inquiryRef, privatePayload);
  } catch (error) {
    console.error('[RE:LIM Q&A] 문의 본문 저장 실패', error);
    const message = firebaseMessage(error, '문의 내용 저장');
    setStatus(writeStatus, message, true);
    submit.disabled = false;
    return;
  }

  setStatus(writeStatus, '문의 내용이 저장되었습니다. 게시판 목록에 반영하고 있습니다.');

  try {
    await setDoc(indexRef, publicPayloadFromPrivate(privatePayload));
    writeForm.reset();
    setStatus(writeStatus, '문의가 정상적으로 등록되었습니다.');
    window.setTimeout(() => {
      closeModal(writeModal);
      window.location.href = `inquiry.html?id=${encodeURIComponent(inquiryRef.id)}`;
    }, 260);
  } catch (error) {
    console.error('[RE:LIM Q&A] 게시판 목록 저장 실패', error);
    // 본문은 삭제하지 않는다. 마이페이지/관리자 기록은 유지하고 다음 로그인 시 목록 복구를 재시도한다.
    const message = `${firebaseMessage(error, '게시판 목록 반영')} 문의 내용 자체는 저장되었습니다. 새로고침 후 다시 확인해 주세요.`;
    setStatus(writeStatus, message, true);
  } finally {
    submit.disabled = false;
  }
...
editForm?.addEventListener('submit', async (event) => {
  ...
  const title = String(data.get('title') || '').trim();
  const content = String(data.get('content') || '').trim();
  if (title.length < 2 || content.length < 5) return setStatus(editStatus, '문의 제목과 내용을 조금 더 입력해 주세요.', true);

  submit.disabled = true;
  const now = Timestamp.now();
  try {
    await updateDoc(doc(db, 'inquiries', activeInquiryId), { category, title, content, updatedAt: now });
    try {
      await updateDoc(doc(db, 'inquiryIndex', activeInquiryId), { category, title, updatedAt: now });
    } catch (indexError) {
      console.warn('[RE:LIM Q&A] 목록 수정 보류', indexError);
    }
    ...
    console.error('[RE:LIM Q&A] 수정 실패', error);
    setStatus(editStatus, firebaseMessage(error, '문의 수정'), true);
  } finally {
    submit.disabled = false;
  }
});
...
deleteButton?.addEventListener('click', async () => {
  ...
  try {
    ...
    try { await deleteDoc(doc(db, 'inquiryIndex', activeInquiryId)); } catch (indexError) { console.warn(indexError); }
    activeInquiryId = null;
    activeInquiry = null;
    closeModal(detailModal);
    history.replaceState(null, '', 'inquiry.html');
  } catch (error) {
    console.error('[RE:LIM Q&A] 삭제 실패', error);
    setStatus(deleteStatus, firebaseMessage(error, '문의 삭제'), true);
  } finally {
...
answerForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    try {
      await updateDoc(doc(db, 'inquiryIndex', activeInquiryId), { status: '답변완료', updatedAt: now });
    } catch (indexError) {
      console.warn('[RE:LIM Q&A] 목록 상태 갱신 보류', indexError);
    }
    activeInquiry = { ...activeInquiry, answer, status: '답변완료', answeredAt: now, updatedAt: now };
    renderDetail(activeInquiryId, activeInquiry, false);
    setStatus(answerStatus, '관리자 답변이 등록되었습니다.');
  } catch (error) {
    console.error('[RE:LIM Q&A] 답변 저장 실패', error);
    setStatus(answerStatus, firebaseMessage(error, '답변 저장'), true);
  } finally {
    submit.disabled = false;
  }
});
