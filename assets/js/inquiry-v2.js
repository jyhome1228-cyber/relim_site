...
const errorMessage = (error, step = '처리') => {
  const code = String(error?.code || 'unknown');
  if (code.includes('permission-denied')) return `${step} 권한이 없습니다. Firestore Rules를 최신 버전으로 게시해 주세요. (${code})`;
...
async function openInquiry(indexItem, syncUrl = true) {
  if (!indexItem) return;
  if (!currentUser) return showPrivacyNotice(true);

  const owner = indexItem.ownerUid === currentUser.uid;
  const admin = isAdmin(currentUser);
  if (!owner && !admin) return showPrivacyNotice(false);
  ...
  try {
    ...
    window.alert(errorMessage(error, '문의 조회'));
  }
}
...
if (db) {
  onSnapshot(
    ...
    (error) => {
      ...
      if (empty) {
        empty.hidden = false;
        empty.textContent = errorMessage(error, '문의 목록 조회');
      }
    }
  );
}
...
writeForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const message = errorMessage(error, '문의 내용 저장');
  ...
  try {
    ...
    console.error('[RE:LIM Q&A] 문의 목록 저장 실패', error);
    try { await deleteDoc(inquiryRef); } catch (rollbackError) { console.error('[RE:LIM Q&A] rollback 실패', rollbackError); }
    const message = errorMessage(error, '게시판 목록 저장');
    setStatus(writeStatus, message, true);
    window.alert(message);
    submit.disabled = false;
    return;
...
editForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    setStatus(editStatus, errorMessage(error, '문의 수정'), true);
  } finally {
    submit.disabled = false;
  }
});
...
deleteButton?.addEventListener('click', async () => {
  ...
  try {
    await deleteDoc(doc(db, 'inquiryIndex', activeInquiryId));
    ...
    setStatus(deleteStatus, errorMessage(error, '문의 삭제'), true);
  } finally {
...
answerForm?.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    console.error('[RE:LIM Q&A] 답변 등록 실패', error);
    setStatus(answerStatus, errorMessage(error, '답변 등록'), true);
  } finally {
    submit.disabled = false;
  }
});
